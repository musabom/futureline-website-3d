/**
 * FinalCTA — closing section. Large headline + dual CTA + entrance spark burst.
 *
 * The spark burst is a one-shot CSS animation triggered by an IntersectionObserver
 * when the section enters the viewport. ~18 absolutely-positioned divs are
 * animated with random end-positions (set as inline CSS variables) so each spark
 * flies in its own direction. No canvas, no JS-per-frame — cheap and crisp.
 */
'use client'
...
interface SparkSpec {
  dx: number // end translate in px
  dy: number
  delay: number // ms
  duration: number // ms
  hue: number // 0..1 — picks accent vs warm
}
...
function generateSparks(): SparkSpec[] {
  ...
  for (let i = 0; i < SPARK_COUNT; i++) {
    ...
    out.push({
      ...
      dy: Math.sin(angle) * dist * 0.7 - 40, // bias upward
...
export function FinalCTA() {
  ...
  // Fire the burst once when the section enters viewport.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      />
      {/* Hairline horizon — echoes the hero grid */}
      <div
        aria-hidden="true"
      ...
      <div className="relative mx-auto max-w-3xl text-center">
        <FadeUp>
          ...
          {/* Headline + spark burst container.
              Sparks emit from the center of the headline outward. */}
              ...
              >
                {sparks.map((s, i) => (
                  <span
                    ...
                    style={
                      {
                        // CSS variables drive the keyframe end-state.
                        '--dx': `${s.dx}px`,
                        '--dy': `${s.dy}px`,
                        ...
                        background:
                          s.hue < 0.6
                            ? 'rgba(186, 230, 253, 0.95)' // cyan
                            : 'rgba(254, 243, 199, 0.95)', // warm
          ...
          <p className="mx-auto mt-6 max-w-xl text-lg text-white/65">
            Start free. No credit card. Cancel any time — though you won&apos;t want to.
          </p>

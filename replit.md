...
- Notification Foundation: Pluggable NotificationProvider interface in src/lib/notifications.ts. Resend provider active (RESEND_API_KEY env var set). Auto-detects: if RESEND_API_KEY exists, uses Resend; otherwise falls back to console logging. Default sender: "FutureLine <onboarding@resend.dev>" (update to verified domain later). Wired into lead stage changes, enrollments, password resets, and automation templates.

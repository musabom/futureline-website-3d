  ...
  return (
    <div className="animate-pulse">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="rounded-xl border border-white/[0.07] bg-slate-950/40 p-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-white/[0.05]" />
              <div>
                <div className="h-8 w-20 bg-white/[0.06] rounded mb-1" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        {[...Array(2)].map((_, i) => (
          <div key={i} className="rounded-xl border border-white/[0.07] bg-slate-950/40 p-6">
            <div className="h-4 w-40 bg-white/[0.06] rounded mb-6" />
            <div className="flex items-end gap-2 h-40">
              {[...Array(7)].map((_, j) => (
                <div key={j} className="flex-1 flex flex-col items-center gap-2">
                  <div className="w-full bg-white/[0.05] rounded-t-md" style={{ height: `${30 + Math.random() * 60}%` }} />
            ...
            <div className="space-y-4">
              {[...Array(5)].map((_, j) => (
                <div key={j} className="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
                  <div>
                    <div className="h-3 w-32 bg-white/[0.06] rounded mb-1.5" />

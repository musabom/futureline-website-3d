...
export default async function InstructorDashboard() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-black text-navy tracking-tight">Welcome, {session.user.firstName}</h1>
        <p className="text-ink-muted mt-1 text-sm">Your instructor dashboard</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        <div className="rounded-xl border border-hairline bg-canvas-card backdrop-blur-sm p-6 lg:col-span-2">
          <div className="flex items-center gap-2 mb-6">
            <BarChart3 size={18} className="text-teal-400" />
            <h2 className="text-2xl font-black text-navy tracking-tight">Enrollment Trend (Last 7 Days)</h2>
          </div>
          <div className="flex items-end gap-2 h-40">
            {trendData.map((day) => (
              <div key={day.label} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-xs font-medium text-ink-muted">{day.count}</span>
                <div
                ...
                />
                <span className="text-[10px] text-ink-muted mt-1">{day.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-hairline bg-canvas-card backdrop-blur-sm p-6">
          <h2 className="text-2xl font-black text-navy tracking-tight mb-4">Approval Status</h2>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-black text-navy">{approvedCount}</span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-black text-navy">{pendingCourses}</span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-black text-navy">{rejectedCount}</span>
            </div>
          </div>
          <div className="mt-6 pt-4 border-t border-hairline">
            <div className="w-full bg-canvas-card rounded-full h-3 overflow-hidden flex">
              {myCourses > 0 && (
                <>
                  <div
            ...
            <p className="text-xs text-ink-muted mt-2 text-center">{myCourses} total courses</p>
          </div>
        </div>
      </div>
      ...
      {coursePerformance.length > 0 && (
        <div className="mb-10">
          <h2 className="text-2xl font-black text-navy tracking-tight mb-4">Course Performance</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {coursePerformance.map((course) => (
              <div key={course.id} className="rounded-xl border border-hairline bg-canvas-card backdrop-blur-sm p-5">
                ...
                <div className="space-y-2">
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-ink-muted">Avg Progress</span>
                      <span className="font-medium text-ink-muted">{course.avgProgress}%</span>
                    </div>
                    <div className="w-full bg-canvas-card rounded-full h-2">
                      <div
                  ...
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="font-medium text-ink-muted">{course.completionRate}%</span>
                    </div>
                    <div className="w-full bg-canvas-card rounded-full h-2">
                      <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="rounded-xl border border-hairline bg-canvas-card backdrop-blur-sm p-6">
          <h2 className="text-2xl font-black text-navy tracking-tight mb-4">My Courses</h2>
          {topCourses.length === 0 ? (
            <p className="text-ink-muted text-sm">No courses yet. Create your first course!</p>
          ) : (
            <div className="space-y-3">
              {topCourses.map((course) => (
                <div key={course.id} className="flex items-center justify-between py-2 border-b border-hairline last:border-0">
                  <div>
                    <div className="text-sm font-medium text-navy">{course.title}</div>
                    <div className="text-xs text-ink-muted">{course._count.lessons} lessons</div>
                  </div>
                  <div className="text-sm text-ink-muted">{course._count.enrollments} students</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="rounded-xl border border-hairline bg-canvas-card backdrop-blur-sm p-6">
          <h2 className="text-2xl font-black text-navy tracking-tight mb-4">Recent Enrollments</h2>
          {recentEnrollments.length === 0 ? (
            <p className="text-ink-muted text-sm">No enrollments yet</p>
          ) : (
            <div className="space-y-3">
              {recentEnrollments.map((enrollment) => (
                <div key={enrollment.id} className="flex items-center justify-between py-2 border-b border-hairline last:border-0">
                  <div>
                    <div className="text-sm font-medium text-navy">{enrollment.user.firstName} {enrollment.user.lastName}</div>
                    <div className="text-xs text-ink-muted">{enrollment.course.title}</div>
                  </div>

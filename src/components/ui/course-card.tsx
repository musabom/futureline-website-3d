  ...
  return (
    <div>
      <motion.div
        ...
        className="group rounded-xl overflow-hidden flex flex-col bg-slate-950/40 border border-white/[0.07] backdrop-blur-sm hover:border-teal-500/30 hover:shadow-xl hover:shadow-teal-500/[0.08] transition-colors duration-300 will-change-transform"
      >
        <Link href={href} className="flex flex-col flex-1">
          {/* Thumbnail */}
          <div className={`h-44 relative overflow-hidden bg-gradient-to-br ${gradient} flex items-center justify-center`}>
            <BookOpen className="text-white/10" size={52} />
          ...
          <div className="flex flex-col flex-1 p-5">
            <h3 className="text-sm font-bold text-white mb-2 group-hover:text-teal-300 transition-colors line-clamp-2 leading-snug">
              {title}
            </h3>
            ...
            <div className="flex items-center gap-3 text-[11px] text-slate-600 mb-5">
              {instructorName && (
            ...
            <div className="mt-auto">
              <div className={`w-full flex items-center justify-between px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest transition-all group-hover:gap-3 ${
                isEnrolled
                  ...
                  : 'bg-white/5 border border-white/10 text-slate-300 group-hover:bg-teal-500/10 group-hover:border-teal-500/20 group-hover:text-teal-400'
              }`}>
                <span>{ctaLabel}</span>
                <span>→</span>
              </div>
